...
const STAGE_TRANSITIONS = {
  ...
  'UNDER_CONTRACT→INSPECTION_PERIOD': { stageNumber: 13, workflow: 'wf_under_contract_tc_handoff', owner: 'TC', name: 'Under Contract → Inspection Period', description: 'TC handshake email DISABLED 2026-06-26 (BGonzalez + Kayla + Monique all paused). 14-day countdown. Day 7 SMS.', automations: [{ type: 'webhook', endpoint: '/webhook/ghl/stage-transition' }, { type: 'set_reminder', reminder_type: 'inspection_day_7_sms', offset_days: 7 }, { type: 'set_reminder', reminder_type: 'inspection_period_end', offset_days: 14 }, { type: 'log', message: 'Stage 13: TC handshake email DISABLED (BGonzalez + Kayla + Monique all paused 2026-06-26). 14-day countdown started. Day 7 SMS scheduled.' }], ghl_actions: ['Send Email', 'Wait', 'Create Task'] },
...
Source of truth: 17C-OH-3.12 transcript — "Seth is going to be underwriting all LOI potential requests so that if it doesn't cash flow at the 1% rule, after looking at..."`;
              ...
              // Seth is paused for now, so keep the workflow artifact but suppress notification delivery.
              const sethEmail = 'claytoninvestmentsolutions@gmail.com';
              const sethId = isPausedRecipientEmail(sethEmail) ? null : await getUserByEmail(sethEmail);
              if (sethId) {
...
First check-in: 3 days from now. Continue until close.`;
              ...
              results.push({ type: 'hand_to_kayla', ok: true, kaylaNotified: false, jaxonNotified: !!jaxonId, montelliMonitoring: true });
            } catch (e) {
