...
export default function Profile() {
  ...
  async function handleConnectGoogle() {
    if (isLocalPreview) {
      setMessage('Google OAuth is not configured for this local preview environment. Use the deployed app to connect Google.');
      return;
    }
  ...
  async function handleSaveScheduling() {
    ...
    setMessage('');
    try {
      const result = await api.updateMe({ scheduling_link: schedulingLink });
      if (result?.user) {
        setUser(result.user);
      }
      ...
      <div style={{
        ...
        {googleStatus?.connected ? (
          ...
          <button
            ...
            disabled={connecting || isLocalPreview}
            style={{
              background: 'white',
              ...
              fontSize: '0.85rem',
              fontWeight: '500',
              cursor: connecting || isLocalPreview ? 'not-allowed' : 'pointer',
              fontFamily: 'inherit',
              display: 'flex',
              alignItems: 'center',
              gap: '0.5rem',
              ...
              opacity: isLocalPreview ? 0.65 : 1,
            }}
            title={isLocalPreview ? 'Google OAuth is disabled in local preview' : undefined}
          >
            ...
            {isLocalPreview ? 'Google OAuth unavailable locally' : connecting ? 'Connecting...' : 'Connect Google Account'}
          </button>
        )}
        ...
        {isLocalPreview && (
          <div style={{ marginTop: '0.5rem', fontSize: '0.72rem', color: 'var(--text-tertiary)' }}>
            Local preview mode blocks Google OAuth so you do not get redirected into a broken auth flow.
          </div>
        )}
      </div>
